...
const showMenu = (menu, validChoices, leave_action, chooseAction) => {
    let input = +readline.question(menu);
    if (validChoices.indexOf(input) === -1) {
        console.log(`Invalid, your input must be one of the following ${console.table(validChoices)}`);
        (0, exports.showMenu)(menu, validChoices, leave_action, chooseAction);
    }
